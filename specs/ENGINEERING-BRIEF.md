...
| **INT-11** | **SFM (Shireburn)** | **Supplier invoice batch — as a generated .xlsx file, uploaded to SFM by hand** | **None. There is no integration: the platform produces the file and a team member uploads it. All reference data comes from Zoho instead (INT-3b, INT-3c). See §A6.** |
...
## A6. SFM (Shireburn) — file handoff, not an integration

> **Decided: there is no SFM integration.** The platform generates the supplier upload as a file; a member of the team uploads it to SFM. Nothing is read from SFM and nothing is posted to it.

**SFM** is the accounting system supplied by **Shireburn** and remains the system of record for supplier payments. The platform's only contract with it is the **file format** — the 22 fixed columns in BE-22.

### INT-11 — What crosses the boundary

| Direction | Payload | How |
|---|---|---|
| **Out** | Supplier invoice batch — the 22-column `.xlsx` defined in BE-22 | Generated by the month-end run (**SD-3468**), emailed to Shared Services, uploaded to SFM **by hand** |
| **In** | — | **Nothing.** No reads, no callbacks, no credentials |

**Reference data comes from Zoho, not SFM.** Supplier and client codes — SFM account number, nominal accounts, entity code, VAT code, payment type, IBAN, BIC, company name — are maintained in Zoho **by our team** (INT-3b, INT-3c, BE-23) and read at generation time.

### BE-26 — File-generation requirements

**Acceptance criteria**
- Reference data is **read from Zoho at export time, never copied into the portal**. A stale local cache of nominal codes produces a silently wrong file.
- Row 1 is the header row **exactly as supplied**; 22 columns, never reordered, renamed or omitted.
- Every constant is a **named config value**, not a literal in the generator. `BATCH` in particular is expected to change per run — confirm with Finance whether SFM expects it incremented.
- A missing mandatory reference field **blocks that row** and names the record, in the month-end email body and in an on-demand download's response, rather than exporting a blank cell that fails on import. **Every other row still generates.**
- Re-generating a **closed** period produces a **byte-identical** file — the safety net that idempotent posting would otherwise have provided.
- The generated file validates against a **real SFM import in a non-production company** before the first live run. With no integration, the format is the entire contract.
- All four reports are downloadable on demand from Internal → Channel & Billing and Internal → Engagements, so Finance is never blocked waiting for the scheduled run.

Generation, scheduling and the on-demand endpoints are specified as a single work item: **SD-3468**.
...
| Weekly satisfaction report (per bench) | Manager | Sent **weekly**. Traffic-light colour **plus** hours used vs total for the bench — e.g. "68 / 80h — 85%". A grey **"What to consider before you rate"** panel lists the three criteria — progress, productivity, responsiveness — as guidance only, above **one** row of three equal-width rating CTAs. **One tap, one submission**; nothing per-criterion is stored. No reply is treated as a green |
