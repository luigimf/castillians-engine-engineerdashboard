...
- **An existing organisation member is granted access immediately — no invitation is created.** The endpoint distinguishes the two on the invitee's address: a known member gets a bench membership row and `emails/11-bench-access-granted.html`; an unknown address gets the invitation path below. **Never a token for someone who already has an account** — a sign-up link is a dead end for them.
- The grant is **live before the email is sent**, so the email announces access rather than offering it. Their bench list (SD-3470) includes it on the next read.
- The email names the role held **on that bench** and states that roles elsewhere are unchanged — Manager and Viewer are per bench (SD-3479).
...
- _Outdated on 23 Sep. Previously: "A bench has exactly one Admin. There is one Admin per client and a bench belongs to exactly one client, so the members response for a bench carries exactly one member with accountRole: "admin" — that client's. Another client's Admin in the same channel is absent from the response, not merely hidden. Scope the roster to the bench's own clientId before applying any access filter; a channel-wide roster is the wrong starting point."_
- **A bench's Admins are its own brand's Admin plus the Admin of every ancestor brand** (SD-3480). The members response carries each of them with `accountRole: "admin"`, own brand first. Admins of sibling or descendant brands are **absent** from the response, not merely hidden. Build the roster from the bench's own `clientId` members **plus the ancestor Admins resolved from the Zoho Parent lineage** — never from a channel-wide roster.
- **Every Admin with access to a bench (own brand or ancestor) is an eligible reviewer on it**, returned in the reviewer list beside the Managers. Eligibility derives from the **account role**, not from a bench membership row, so there is nothing to grant and revoking bench access cannot remove it. Assert that an Admin appears as eligible on a bench they hold no membership row for.
