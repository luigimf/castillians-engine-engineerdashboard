...
- `pageSize` is **5**, a named constant, and clamped server-side if a larger value is requested.
- Sort is **most recent first, tie-broken on entry id** — two entries on the same date must not swap between pages.
- A page beyond the last returns the **last** page.
- The calendar endpoint is deliberately **unpaginated** and returns the whole month plus `earliestMonth` / `latestMonth` so the client can bound its navigation.
- `PATCH` returns the updated entry **and** its full history, so the accordion needs no second call.
